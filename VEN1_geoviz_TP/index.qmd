...
-   Sur votre ordinateur, creez un dossier **TP_geoviz** à l'emplacement de votre choix.
...
-   Dans ce repertoire, créez également un sous répertoire **data** pour stocker les données.
...
Un chunk contanant une seule cellule
...
Un chunk contanant trois cellules
...
Téléchargez les données et placez-les-les dans votre repertoire **data**.
...
La carte est un peu grande. Nous pouvons la redimensionner en utilisant le parametre `svg_width`.
...
L'attribut `svg_width` modifie les parametres du containeur SVG contenant la couche. Nous verrons plus tard comment fonctionnement les containeurs.
:::

## 3.4 Personnaliser l'affichage

La bibliothèque `geoviz` n'est pas seulement un viewer de couches SIG. C'est un outil pour réaliser des cartes vertorielles. Avec `geoviz`, les cartes sont dessinées au format SVG. Il est donc possible d'utiliser tous les attributs SVG pour modifier la carte. A une nuance près. Le JavaScript ne support pas les noms de variable avec un tiret. Il est donc d'usage de convertir ce tiret en *camelCase*. Par exemple : `stroke-width` donnera `strokeWidth`.

Vous pouvez essayer :

-   `fill` : couleur de fond
-   `stroke` : couleur de contour
-   `strokeWidth` : epaisseur des lignes
...
Rappelez-vous qu'avec Observable, nous sommes dans un environnement interactif et réacif. On peut donc mettre en place des interactions pour personnaliser la carte. <a href="https://observablehq.com/documentation/inputs/overview"><img src="img/logo_doc.png"/></a>
...
## 4. Logiques et principe de la blibliothèque `geoviz`

### 4.1 Documentation et exemples

La documettaion de `geoviz` est disponible à l'adresse suivante : <a href="https://riatelab.github.io/geoviz/" target = "_BLANK">riatelab.github.io/geoviz</a>. Cliquez sur ce lien, et conservez précieusement la page pour pouvoir vous y referer à tout moment.
...
Au même titre que l'instruction `path`, la bibliothèque `geoviz` met à disposition un certain nombre de `marks` permetant de constituer une carte.
...
Mais la plupart du temps, on utilisera ces `marks` à l'interieur de conteneurs dans lesquels nous pourrons les superposer.
...
Pour bien fonctionner, le conteneur a besoin que vous definissiez une `projection` et/ou une emprise géographie (`domain`).
...
Dans l'ecosystème de `d3js` et `geoviz`, on utilise des fonctions de projections bien spécifiques dédiées à la représentation de données. elles sont réaparties dans 3 bibliothèques : `d3-geo` <a href="https://github.com/d3/d3-geo" target = "_BLANK"><img src="img/logo_doc.png"/></a>, `d3-geo-projection` <a href="https://github.com/d3/d3-geo-projection" target = "_BLANK"><img src="img/logo_doc.png"/></a> et `d3-geo-polygon` <a href="https://github.com/d3/d3-geo-polygon" target = "_BLANK"><img src="img/logo_doc.png"/></a>.
...
Avec les containeurs, on peut maintenant empiler les couches. En jouant avec les `marks` et les attributs SVG, on peut réaliser de très beaux templates cartographiques.
...
Le tableau contient des informations à plusieurs dates. Il y a donc plusieurs fois le même identifiant (id) dans le tableau de données. La première étaoe consiste donc à selectionner une année.
...
Avec `method`, vous pouvez changer la méthode de discretisation : 'quantile', 'q6', 'equal', 'jenks', 'msd', 'geometric', 'headtail', 'pretty', 'arithmetic' ou 'nestedmeans'.
...
### 5.6 Tout est paremetrable/configurable
...
Depuis la version 1.4, Quarto propose des mises en page à la façon de dashboards. Tout ce que nous avons vu précédemment est donc facilement mobilisable pour réaliser une application complète. Queleques exemples sont disponibles sur le site de Quarto.
